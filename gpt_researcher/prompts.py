...
- If any requested information such as phone numbers or email addresses is not actually present in the provided context, explicitly write '-' or 'nil' instead of inventing, fabricating, or using placeholder/example data.
- If all values in a table row or column for a given section are '-' or 'nil', remove that row/column entirely from the final output to avoid empty-looking sections. If an entire table would be empty, omit the table and instead provide a descriptive note stating that no valid data was found, optionally listing the entities for which data was missing.
- {reference_prompt}
- {tone_prompt}
...
"""
    ...
    @staticmethod
    def curate_sources(query, sources, max_results=50):
