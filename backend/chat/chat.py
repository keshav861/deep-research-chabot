...
class ChatAgentWithMemory:
    ...
    async def chat(self, message, websocket):
        ...
        try:
            if isinstance(message, dict):
                # Use regular message content if in dict format
                message = message.get("message", message)

            # Default chat behavior
            message = f"""
             You are SE Deep Researcher, a autonomous research agent created at SE.

             This is a chat message between the user and you: SE Deep Researcher.
    ...
    async def _stream_output(self, type, label, data, websocket):
        """Helper method to stream output back to the client"""
        if websocket is not None:
