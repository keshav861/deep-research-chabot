...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        if self.researcher.cfg.curate_sources:
            ...
            curated_data, curator_decisions = await self.researcher.source_curator.curate_sources(research_data)
            self.researcher.context = curated_data
            # Store curator decisions for logging
            self.researcher.curator_decisions = curator_decisions
            
            # Send curator decisions to websocket for logging
            if self.researcher.websocket:
                await self.researcher.websocket.send_json({
                    "type": "curator_decisions",
                    "decisions": curator_decisions
                })
