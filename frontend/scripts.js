const GPTResearcher = (() => {
  ...
  let isStopping = false; // Flag to track if we're in the process of stopping

  const init = () => {
    console.log("Initializing GPT Researcher application...");
    ...
    // Initialize history panel functionality BEFORE loading history
    console.log("Initializing history panel...");
    initHistoryPanel();

    // Initialize WebSocket monitoring panel BEFORE any connections
  ...
  const initHistoryPanel = () => {
    ...
    if (historyPanelOpenBtn && historyPanel) {
      historyPanelOpenBtn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
      ...
      console.error("History panel elements not found:", {
        historyPanelOpenBtn: !!historyPanelOpenBtn,
        historyPanel: !!historyPanel
      });
    }

    if (historyPanelToggle && historyPanel) {
  ...
  const initWebSocketPanel = () => {
    ...
    if (!websocketPanel || !websocketPanelOpenBtn || !websocketPanelToggle) {
      console.error("WebSocket panel elements not found:", {
        websocketPanelOpenBtn: !!websocketPanelOpenBtn,
        websocketPanel: !!websocketPanel,
        websocketPanelToggle: !!websocketPanelToggle
      });
      return;
    }
    ...
    websocketPanelOpenBtn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      console.log("Opening WebSocket panel - current classes:", websocketPanel.className);
      websocketPanel.classList.add('open');
      console.log("After opening - classes:", websocketPanel.className);
    });
  ...
  const initAuth = () => {
    ...
    // Clear sensitive data when page is unloaded (security measure)
  ...
  const showLoginModal = () => {
    ...
    // Clear password field for security when showing login modal
    const passwordInput = document.getElementById('passwordInput');
    if (passwordInput) {
      passwordInput.value = '';
    }
  ...
  const clearSensitiveData = () => {
    ...
    // Clear any error messages that might contain sensitive info
    const errorDiv = document.getElementById('loginError');
    if (errorDiv) {
      errorDiv.textContent = '';
    }

    // Note: We don't clear authToken here as it's needed for session persistence
    // The token will be validated on next page load
  };
  ...
  const renderHistoryEntries = () => {
    ...
    conversationHistory.forEach((entry, index) => {
      ...
      // Prevent download links from triggering the entry click event
      const downloadLinks = entryElement.querySelectorAll('.history-entry-format a');
      downloadLinks.forEach(link => {
        link.addEventListener('click', (e) => {
          e.stopPropagation(); // Prevent the click from bubbling up to the entry
        });
      });
  ...
  // Function to handle pondering questions
  const handlePonderingQuestions = (questions) => {
    const container = document.getElementById('ponderingQuestionsContainer');
    const questionsContainer = document.getElementById('questionsContainer');

    if (!container || !questionsContainer) {
      console.error('Could not find pondering questions containers');
      return;
    }
    ...
    if (validQuestions.length === 0) {
      console.log('No valid questions to display');
      container.style.display = 'none';
      return;
    }

    // Add each question as a rounded box
    validQuestions.forEach(question => {
      const questionBox = document.createElement('div');
      questionBox.className = 'question-box';
      // Remove any quotes if they exist and trim whitespace
      const cleanQuestion = question.replace(/^["']|["']$/g, '').trim();
      questionBox.textContent = cleanQuestion;
      questionsContainer.appendChild(questionBox);
    });

    // Show the container and ensure it's visible
    container.style.removeProperty('display');
    container.style.display = 'block';

    // Scroll the questions into view
  ...
  const startResearch = () => {
    ...
    if (isResearchActive) {
      showToast('Research is already in progress. Please stop it first.', 3000);
      return;
    }

    // Check if we're in the process of stopping
    if (isStopping) {
      showToast('Please wait for the current research to stop completely.', 2000);
      return;
    }

    console.log('Starting new research...');

    // Reset the active research state first
    isResearchActive = false;
    lastRequestData = null;
    reconnectAttempts = 0;

    // Hide the pondering questions container
    const ponderingContainer = document.getElementById('ponderingQuestionsContainer');
    if (ponderingContainer) {
      ponderingContainer.style.display = 'none';
    }
    ...
    // Dispose of any existing socket connection
    if (dispose_socket) {
      dispose_socket();
    }
    ...
    // Show stop button, disable submit button
    const stopButton = document.getElementById('stopButton');
    ...
    // Create new WebSocket connection and start research
  ...
  const stopResearch = () => {
    ...
    // Send stop signal to backend BEFORE closing socket
    try {
      if (socket && socket.readyState === WebSocket.OPEN) {
        socket.send(`stop ${JSON.stringify({ reason: 'user_request' })}`);
    ...
    // Small delay to ensure stop signal is sent
    setTimeout(() => {
      // Terminate ongoing connection
      dispose_socket?.();
      ...
      // Clear progress and report
      document.getElementById('output').innerHTML = '';
      document.getElementById('reportContainer').innerHTML = '';

      // Hide pondering questions
      const ponderingContainer = document.getElementById('ponderingQuestionsContainer');
      if (ponderingContainer) {
        ponderingContainer.style.display = 'none';
      }
      ...
      // Hide stop button, show submit button
      const stopButton = document.getElementById('stopButton');
      const submitButton = document.getElementById('submitButton');
      if (stopButton) {
      ...
      // Stop research icon spinning
      updateResearchIcon(false);

      // Update WebSocket status
      updateWebSocketStatus();

      showToast('Research stopped. Ready for new research.', 3000);

      // DON'T reopen WebSocket connection here - let startResearch do it
      // This prevents automatic restart
      isStopping = false;

      console.log('Research stopped successfully');

    }, 150); // 150ms delay to ensure stop signal is sent
  }
  ...
  const listenToSockEvents = () => {
    ...
    // Configure Showdown converter with enhanced formatting options
    const converter = new showdown.Converter({
      ...
      parseImgDimensions: true,   // Parse image dimensions from attributes
      omitExtraWLInCodeBlocks: true, // Omit trailing newline in code blocks
      // Image parsing removed
    });

    // Fix issues with code block and table formatting
    ...
    socket.onmessage = (event) => {
      ...
      // Handle all log messages for research progress
      if (data.type === 'logs') {
        if (data.content === 'subqueries' && data.output) {
          console.log('Found subqueries:', data);  // Debug log
          // Check if output contains the queries
          if (data.metadata && Array.isArray(data.metadata)) {
            console.log('Processing questions:', data.metadata);  // Debug log
            handlePonderingQuestions(data.metadata);
            // Make the pondering questions section visible since we have questions
            const ponderingContainer = document.getElementById('ponderingQuestionsContainer');
            if (ponderingContainer) {
              ponderingContainer.style.display = 'block';
            }
          } else {
            console.log('No valid metadata found:', data.metadata);  // Debug log
          }
        }
        // Add the message to output container with formatting
        if (data.output) {
          const progressContainer = document.getElementById('output');
          if (progressContainer) {
            const output = data.output;

            // Skip duplicate scraping messages
            if (output.includes('Scraping content from 5 URLs')) {
              const lastMessage = progressContainer.lastElementChild;
              if (lastMessage && lastMessage.textContent.includes('Scraping content from')) {
                // Update existing scraping message instead of adding new one
                lastMessage.innerHTML = '🌐 Consolidating content from multiple sources...';
                return;
              }
            }
            ...
            messageDiv.className = 'agent_response';
            ...
            const messageType =
              content.includes('error') ? 'error' :
                content.includes('warning') ? 'warning' :
                  content === 'research_started' || content === 'research_completed' ? 'progress' :
                    'info';
            ...
            // Only auto-scroll the message container, not the whole page
        ...
        if (report_type === 'multi_agent') {
          // For multi-agent: Replace the previous report with the latest one
          allReports = data.output;

          // Clear the report container to ensure we don't show any drafts
          const reportContainer = document.getElementById('reportContainer');
          if (reportContainer) {
            reportContainer.innerHTML = '';
          }
        } else {
          // For other report types: Accumulate reports to show continuous updates
          // This preserves the original behavior for non-multi-agent reports
          currentReport = data.output;

          // Write the current report to the container (append mode for continuous display)
          const reportContainer = document.getElementById('reportContainer');
          if (reportContainer) {
            writeReport(data, converter, false, true); // Not final, append=true
          }

          // Also update allReports for the final display
          allReports += data.output;
        }
        ...
        // Now that research is complete, display the final accumulated report
        if (allReports) {
          const report_type = document.querySelector('select[name="report_type"]').value;
          const finalData = { output: allReports, type: 'report' };

          // For multi-agent, we've already accumulated the final report
          // For other report types, we've been appending to the UI during generation
          if (report_type === 'multi_agent') {
            writeReport(finalData, converter, true, false); // isFinal=true, append=false
          } else {
            // For other report types, we've been showing updates continuously
            // No need to rewrite the entire report
          }
        }
    ...
    socket.onopen = (event) => {
      ...
      // If we're stopping, don't start anything
      if (isStopping) {
        console.log("Connection opened during stop operation, not sending start command");
        return;
      }
      ...
      // If research is not active, don't send start command
      // This prevents auto-start on connection
      if (!isResearchActive) {
        console.log("Connection opened but no active research, waiting for user to start");
        return;
      }
    ...
    socket.onclose = (event) => {
      ...
      // Don't reconnect if we're stopping or if it's a normal closure
      if (isStopping) {
        console.log('Connection closed due to stop operation');
        return;
      }

      // Only attempt reconnection if:
      // 1. Research is active
      // 2. Connection wasn't closed intentionally (code 1000 is normal closure)
      // 3. We haven't exceeded max reconnect attempts
      if (isResearchActive && event.code !== 1000 && reconnectAttempts < maxReconnectAttempts) {
        console.log('Unexpected connection close during research, attempting reconnection...');
        reconnectWebSocket();
  ...
  const updateState = (state) => {
    ...
    switch (state) {
      case 'in_progress':
        ...
        // Reset last request data when starting new research
        lastRequestData = null;
      ...
      case 'finished':
        ...
        isResearchActive = false;
        // Hide Stop button, enable submit button
        const stopButton = document.getElementById('stopButton');
        const submitButton = document.getElementById('submitButton');
        if (stopButton) {
          stopButton.style.display = 'none';
      ...
      case 'error':
        ...
        isResearchActive = false;
        // Hide Stop button, enable submit button
        const errorStopBtn = document.getElementById('stopButton');
        const errorSubmitBtn = document.getElementById('submitButton');
        if (errorStopBtn) {
          errorStopBtn.style.display = 'none';
        }
        if (errorSubmitBtn) {
      ...
      case 'initial':
        ...
        isResearchActive = false;
        // Hide Stop button, enable submit button
        const initialStopBtn = document.getElementById('stopButton');
        const initialSubmitBtn = document.getElementById('submitButton');
        if (initialStopBtn) {
          initialStopBtn.style.display = 'none';
        }
        if (initialSubmitBtn) {
  ...
  function setCookie(name, value, days) {
    ...
    if (cookieSize > MAX_COOKIE_SIZE) {
      ...
      showToast('Warning: History too large for storage! Older entries will be removed.');
      ...
      if (name === 'conversationHistory') {
        try {
          ...
          if (Array.isArray(historyData) && historyData.length > 1) {
            ...
            const reducedHistory = historyData.slice(0, 10);
...
- Click Cancel to REPLACE all existing history with imported data`)) {
  ...
  const addChatMessage = (message, isUser = false) => {
    ...
    if (!isUser) {
      // Use showdown for markdown conversion with enhanced formatting
      const converter = new showdown.Converter({
        ghCodeBlocks: true,
        tables: true,
        tasklists: true,
