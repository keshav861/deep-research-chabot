...
from fastapi import FastAPI, Request, WebSocket, WebSocketDisconnect, File, UploadFile, BackgroundTasks, HTTPException, Depends, status
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allow all origins for Azure deployment
...
def create_session_token() -> str:
    ...
    active_sessions[token] = datetime.now() + timedelta(hours=24)  # 24 hour expiry
    return token
...
def cleanup_expired_sessions():
    ...
    expired_tokens = [token for token, expiry in active_sessions.items() if current_time > expiry]
...
@app.post("/report/")
async def generate_report(research_request: ResearchRequest, background_tasks: BackgroundTasks, token: str = Depends(get_current_user)):
...
async def delete_file(filename: str, token: str = Depends(get_current_user)):
